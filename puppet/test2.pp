class install_app{
$web = [ "ruby", "rubygems", "ImageMagick", "git", "expect", "mysql" ]
...
file { 'install_gems.sh':
       path => '/home/install_gems.sh',
       ensure  => file,
       source => "puppet:///files/install_gems.sh",
       mode    => 0777,
       owner   => 'root',
       group   => 'root',
...
file { 'auth.yml':
    ...
    owner   => 'root',
    group   => 'root',
    require => Exec["/home/install_gems.sh"],
    replace => true
}
file { 'config.yml':
